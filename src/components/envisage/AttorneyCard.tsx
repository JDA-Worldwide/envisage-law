...
export default function AttorneyCard({ attorney, showNiche }: AttorneyCardProps) {
  const body = (
    <>
      ...
      <div className="border-t-[3px] border-t-brand-secondary px-[22px] py-[22px] pb-[26px]">
        ...
        {attorney.hasProfile ? (
          <span className="mt-4 inline-flex items-center gap-[7px] text-xs font-bold uppercase tracking-[0.08em] text-brand-secondary-dark">
            View profile
          ...
          <span className="mt-4 inline-block text-[11.5px] font-semibold italic tracking-[0.04em] text-brand-muted">
  ...
  if (attorney.hasProfile) {
    return (
      <Link href={attorney.href} className="group block overflow-hidden rounded-md border border-brand-border bg-white transition-all duration-200 hover:-translate-y-[5px] hover:shadow-md hover:border-transparent">
        {body}
      </Link>
    );
  }
