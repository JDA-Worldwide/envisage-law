import type { RecognitionBandProps } from "./types";
    ...
    <section className="border-y border-brand-border bg-brand-surface">
      <div className="mx-auto max-w-content px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-[980px] flex-wrap items-stretch overflow-hidden rounded-xl border border-brand-border bg-white shadow-sm">
          <div
            className="flex flex-1 items-center gap-[18px] border-r border-brand-border px-9 py-[30px]"
            style={{ flexBasis: 320 }}
          >
            ...
            <div className="flex flex-col gap-1">
              {year && (
                <span className="text-[11px] font-bold uppercase tracking-[0.16em] text-brand-secondary-dark">
                  {year}
                </span>
              )}
              <span className="text-[17px] font-extrabold leading-[1.25] text-brand-primary">
                {title}
              </span>
            </div>
          </div>
          {tiers && tiers.length > 0 && (
            <div className="flex" style={{ flex: "2 1 420px" }}>
              {tiers.map((t, i) => (
                <div
                  key={t._key}
                  className={`flex flex-1 flex-col items-center justify-center gap-[7px] px-4 py-[30px] text-center ${i > 0 ? "border-l border-brand-border" : ""}`}
                >
                  ...
                  <span className="text-[15px] font-bold leading-[1.3] text-brand-primary">
                    {t.label}
                  </span>
                </div>
              ))}
